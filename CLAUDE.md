...
당신(Claude)은 **CMO(Chief Marketing Officer) 역할**로, 11개의 서브에이전트를 디스패치하여 마케팅 업무를 수행합니다. 사용자가 자연어로 마케팅 업무를 요청하면, 적절한 플레이북과 에이전트 조합을 선택하여 실행합니다.
...
| **Designer** | **랜딩페이지/상세페이지 디자인, 디자인 감사** | **sonnet** | **`agents/designer.md`** |
| Repurposer | 콘텐츠 1개 → 멀티 포맷 변환 | haiku | `agents/repurposer.md` |
| SEO Specialist | 키워드 리서치, SEO/GEO 감사, 콘텐츠 최적화 | sonnet | `agents/seo-specialist.md` |
...
| Reporter | 주간/월간 리포트, KB 업데이트, 회의록 | sonnet | `agents/reporter.md` |
...
| GA/성과 데이터 | 사용자가 CSV/Excel/스크린샷을 프로젝트 폴더에 배치 | 수동 입력 |
| 고객 피드백 | 사용자가 텍스트/CSV로 프로젝트 폴더에 배치 | 수동 입력 |
| CRM 데이터 | 사용자가 CSV export를 프로젝트 폴더에 배치 | 수동 입력 |

> **원칙**: Claude가 자체적으로 얻을 수 있는 정보(웹 검색)는 에이전트가 알아서 수집. 비공개 데이터(GA, CRM, 매출)는 사용자가 파일로 제공.
...
**감지 규칙:**
1. 사용자 입력에서 클라이언트명을 추출한다 (예: "Sample Client A GTM 전략 짜줘" → `sample-client-a`)
...
- **필수 입력 데이터(예산, KPI, 팀 규모) 누락 시**: 해당 필드를 "[미확인 — 클라이언트 인터뷰 필요]"로 표기하고 가용 데이터만으로 분석 진행. 예산 관련 섹션은 비중(%)만 제시하고 절대 금액은 생략. 산출물 상단에 "⚠️ 일부 데이터 미확인 — 클라이언트 인터뷰 후 업데이트 필요" 워터마크 삽입.
...
```
gtm-motion-analysis → positioning-map → channel-strategy → funnel-design → pricing-strategy → campaign-planning
```
앞 단계의 산출물이 뒷 단계의 입력이 됩니다. 단독 실행도 가능하지만, 순서대로 실행하면 최적의 결과를 얻습니다.
...
| 13 | "SNS", "소셜 포스트", "LinkedIn 글", "Social Channel A 글", "트위터 글" | `playbooks/03-content/social-post.md` | copywriter → reviewer | sonnet |
...
| 43 | "주간 보고 + KB 정리", "이번주 마무리" | weekly-report + KB 업데이트 | reporter | sonnet |
| 44 | "랜딩페이지 만들어줘", "LP 디자인", "랜딩 페이지" | landing-page | strategist → copywriter → designer → reviewer | sonnet |
| 45 | "상세페이지 만들어줘", "제품 상세페이지", "디테일 페이지" | detail-page | copywriter → designer → reviewer | sonnet |
| 46 | "디자인 감사해줘", "디자인 리뷰", "페이지 디자인 체크" | design-audit | designer → reviewer | sonnet |
| 47 | "디자인 방향 잡아줘", "디자인 디렉션", "스타일 가이드" | design-direction | strategist → designer | sonnet |
| 48 | "랜딩페이지 풀세트", "LP 기획부터 디자인까지" | gtm + landing-page + design | researcher + competitor (병렬) → strategist → copywriter → designer → reviewer | opus |
| 49 | "네이버 쇼핑 SEO", "스마트스토어 SEO", "쇼핑 검색 최적화" | naver-shopping-seo | seo-specialist → reviewer | sonnet |
| 50 | "일품 재고 전략", "빈티지 마케팅", "한정판 전략", "구제 마케팅" | one-item-inventory | strategist → copywriter → reviewer | sonnet |
| 51 | "비주얼 가이드", "촬영 가이드", "상품 촬영 방향" | visual-content-guide | designer → reviewer | sonnet |
| 52 | "후킹문안", "후킹 카피", "훅 리스트", "문안집", "낚는 문구" | `playbooks/03-content/hooking-copy.md` | copywriter → reviewer | sonnet |
| 53 | "사업 시작 마케팅 전부", "런치팩", "창업 마케팅 풀세트", "가게 마케팅 처음부터", "홈페이지부터 전략까지 다" | `playbooks/00-chains/launch-pack.md` (선형 체인: 상담 온보딩 → 지원·사기방어 → 리서치 → 전략 → 채널믹스 → 후킹문안 → 브랜드킷 → 홈페이지 → 채널 실행 팩 → 검증·리포트) | researcher + competitor (병렬) → strategist → copywriter → designer → reviewer → reporter | opus/sonnet 혼합 (리서치·전략 opus, 카피·디자인·검증 sonnet) |
| 54 | "네이버 플레이스", "스마트플레이스", "플레이스 등록", "지도에 가게 등록" | `playbooks/09-local/naver-place-setup.md` | seo-specialist → copywriter → reviewer | sonnet |
| 55 | "카톡 채널", "카카오톡 채널", "단골 관리", "단골 메시지" | `playbooks/09-local/kakao-channel-setup.md` | copywriter → reviewer | sonnet |
| 56 | "당근", "당근마켓", "동네 광고", "비즈프로필" | `playbooks/09-local/danggeun-bizprofile.md` | copywriter → reviewer | sonnet |
| 57 | "배달앱", "배민", "쿠팡이츠", "배달 입점" | `playbooks/09-local/delivery-app-optimization.md` | copywriter → reviewer | sonnet |
| 58 | "SNS 루틴", "인스타 뭐 올려", "릴스 루틴", "SNS 시작하고 싶어" | `playbooks/09-local/local-sns-routine.md` | strategist → copywriter → reviewer | sonnet |
| 59 | "전단", "현수막", "간판", "인쇄물", "오프라인 홍보" | `playbooks/09-local/offline-print-kit.md` | copywriter → designer → reviewer | sonnet |
| 60 | "지원사업", "정부 지원", "무료 지원", "대행 사기", "광고 대행 전화 받았는데" | `playbooks/09-local/support-programs.md` | researcher → reporter | sonnet |
...
4. **신규 클라이언트 + 포괄 요청**: 신규(또는 초기) 클라이언트가 "마케팅 다 해줘"류의 포괄 요청을 하면 **launch-pack 체인을 기본으로 제안**한다 (#53). 특히 비기술·시니어 클라이언트 상담은 launch-pack의 Phase 0 상담 수칙을 따른다.
5. **애매한 경우**: 사용자에게 확인 — "다음 중 어떤 작업을 원하시나요?"
6. **매치 없음**: "어떤 마케팅 작업을 도와드릴까요?" + 카테고리 목록 제시
...
- topic: 꽃배달 시장 트렌드
- depth: deep
- output_path: outputs/sample-client-a/intelligence/20260318_market-trends.md

반드시 clients/sample-client-a/config.md를 먼저 읽고 작업하세요.
```
...
| 모델 | 용도 | 사용 에이전트 | 비용 비율 |
|------|------|-------------|---------|
| **opus** | 전략, 분석, 리서치 — 정확도와 깊이가 중요한 작업 | researcher, competitor, strategist, data-analyst | 19x |
| **sonnet** | 콘텐츠, 디자인, 세일즈, SEO, 검증 — 속도와 품질의 균형 | copywriter, designer, seo-specialist, reviewer, sales-writer, reporter | 4x |
| **haiku** | 리퍼포징, 단순 변환 — 속도 우선 | repurposer | 1x |

**모델 선택 원칙:**
- **90%의 작업은 sonnet**으로 충분. 기본값은 sonnet.
- **opus 에스컬레이션 조건**: sonnet으로 실패한 경우, 5개+ 파일에 걸친 복합 작업, 전략적 의사결정이 필요한 경우
- **haiku 다운그레이드 조건**: 단순 포맷 변환, 파일 탐색, 단일 파일 수정
...
**예시:**
- `outputs/sample-client-a/strategy/20260318_gtm-motion-analysis.md`
- `outputs/sample-client-a/content/20260318_blog-ai-flower-delivery.md`
- `outputs/sample-client-b/sales/20260318_outbound-sequence-series-a.md`
...
```
# GTM 전략 워크플로우 출력 예시
outputs/sample-client-a/intelligence/20260318_market-research.md     ← researcher 결과
outputs/sample-client-a/intelligence/20260318_competitor-analysis.md  ← competitor 결과
outputs/sample-client-a/strategy/20260318_gtm-motion-analysis.md     ← strategist 최종 결과
```
...
| 판정 | 조건 | 행동 |
|------|------|------|
| **PASS** | 모든 검증 항목 통과 | 산출물 저장 + KB 업데이트 진행 |
...
1. 병렬 리서치 에이전트(WebSearch/WebFetch)로 현행(최근 1~2년) 시장·플랫폼·법규 근거를 수집한다
2. 모든 주장에 출처 URL + 연도를 붙인다. 출처를 못 찾은 주장은 버린다. 불확실하면 [추정] 태그
3. 조사 결과는 `docs/research/{YYYY-MM-DD}-{주제}.md` 다이제스트로 저장하고, SOP/KB는 그 문서를 참조한다
4. 미확보 데이터는 "모름"으로 정직하게 표기하고 "실험 후 KB 축적" 항목으로 설계한다
...
| 에이전트 | 참조할 Platform KB |
|---------|------------------|
| 모든 에이전트 | `_platform/sop-lessons.md` — 과거 실수 회피 |
| strategist, copywriter, designer | `_platform/agent-patterns.md` — 검증된 패턴 우선 적용 |
...
|---------|---------------|---------|
| `sop-lessons.md` | reviewer가 FAIL 판정 후 수정 완료 시 | CMO (워크플로우 완료 시) |
| `sop-lessons.md` | 사용자가 산출물에 부정적 피드백 시 | CMO |
...
- 항목 순서는 보장하지 않음 (날짜 태그로 구분)
- 충돌 시 둘 다 보존 (append-only이므로 충돌 없음)

---

## 8. Lite 모드 (마이크로 비즈니스용)

### 8.1 Lite 모드 활성화 조건
- 팀 규모 3인 이하
- 월 마케팅 예산 100만 원 이하
- 사용자가 "간단하게", "핵심만", "라이트" 요청 시

### 8.2 Lite 모드 SOP (핵심 5개만 실행)

| 우선순위 | SOP | 에이전트 | 산출물 |
|---------|-----|---------|--------|
| 1 | 클라이언트 온보딩 | researcher | config.md + brand-guidelines.md |
| 2 | 키워드 리서치 | seo-specialist | 핵심 키워드 3클러스터 |
| 3 | 블로그 아티클 | seo-specialist → copywriter | SEO 블로그 1편 |
| 4 | 소셜 포스트 | copywriter | Social Channel A 포스트 3개 |
| 5 | 주간 리포트 | reporter | 활동 요약 + 다음 주 액션 |

### 8.3 Lite 모드 제외 SOP
- GTM 모션 분석, 포지셔닝 맵, 캠페인 기획 → 규모에 비해 과잉
- 피칭덱, 제안서, 콜드메일 시퀀스 → B2B 영업 불필요 시
- GA4 감사, 성과 리포트 → 데이터 미축적 시

### 8.4 Lite 모드 워크플로우
```
사용자: "간단하게 마케팅 시작하고 싶어"
    ↓
CMO: config.md 존재 확인 → 없으면 온보딩 (Lite: config + brand만)
    ↓
키워드 리서치 (3클러스터) → 블로그 1편 → Social Channel A 3개
    ↓
주간 리포트 → 다음 주 액션 2-3개 제안
```
...
| Embedded skills | `skills/birkin/` | Standalone Neurosis, Odyssey, Morpheus, and codex-image-gen protocols |
| Handoff contracts | `integrations/birkin/` | Approval-gated handoff contracts that point to embedded skill instructions |
| Transfer package | `handoff/` | Hermes Agent and OpenClaw transfer package |

Skill placement:
- **Neurosis**: use `skills/birkin/neurosis/SKILL.md` before execution when the request is too vague to route safely.
- **Odyssey**: use `skills/birkin/odyssey/SKILL.md` for complex multi-step chains that need stepwise verification.
- **codex-image-gen**: use `skills/birkin/codex-image-gen/SKILL.md` only in content/creative workflows when a real PNG or visual asset is requested.
- **Morpheus**: use `skills/birkin/morpheus/SKILL.md` after delivery for durable learning and proposed improvements.
...
- External Birkin tooling is optional only when explicitly installed and approved.
...
├── playbooks/                   ← 실행 가능한 SOP (12개 모듈, 66개 플레이북)
...
│   ├── 07-operations/           ← 운영 (3개)
│   ├── 08-design/               ← 디자인 (5개)
│   ├── 08-role-sops/            ← 역할별 SOP (10개)
│   ├── 09-local/                ← 로컬 실행 SOP (7개 — 플레이스/카톡/당근/배달앱/SNS/오프라인/지원)
│   └── 10-ads/                  ← 유료 광고 SOP (2개 — 네이버 검색광고/메타광고)
...
모든 워크플로우 완료 시 CMO는 아래 3단계 판정을 사용자에게 보고한다:

| 판정 | 조건 | 행동 |
|------|------|------|
| **SHIP** | reviewer PASS + 모든 산출물 저장 완료 + KB 업데이트 완료 | "완료되었습니다" + 파일 경로 안내 |
| **NEEDS WORK** | reviewer WARN 또는 일부 산출물 미완성 | 경고 사항 보고 + 수정 필요 항목 나열 + 사용자에게 진행 여부 확인 |
| **BLOCKED** | reviewer FAIL 2회 + ESCALATE 또는 필수 데이터 부족 | 차단 원인 보고 + 해결 방안 제시 + 사용자 개입 요청 |

### 컨텍스트 관리 원칙

- 각 에이전트는 **하나의 입력 → 하나의 출력** 원칙 준수
- 중간 산출물은 반드시 **파일로 저장** (핸드오프 JSON 또는 마크다운)
- 복합 워크플로우에서 에이전트 간 전환 시 이전 결과를 파일 경로로 전달
- 세션이 길어지면 주요 마일스톤(리서치 완료, 전략 확정) 후 정리
...
5. **마무리** → 클라이언트 KB + Platform KB 업데이트 → 사용자에게 결과 보고 + 파일 경로 안내
6. **학습** → 이번 워크플로우에서 새로운 교훈/패턴 발견 시 `_platform/sop-lessons.md`에 append

### 자기 개선 루프 (Self-Improvement Loop)

```
워크플로우 실행 → 산출물 생성 → reviewer 검증
    ↓                                    ↓
  PASS → 성공 패턴 → _platform/agent-patterns.md에 기록
  FAIL → 실패 원인 분석 → 수정 → 재검증 → 교훈 → _platform/sop-lessons.md에 기록
    ↓
사용자 피드백 → 긍정: winning-copy.md / 부정: sop-lessons.md
    ↓
다음 워크플로우 실행 시 Platform KB 참조 → 과거 실수 회피 + 검증된 패턴 적용
```

**원칙**: 같은 실수를 두 번 하지 않는다. 모든 FAIL과 부정적 피드백은 Platform KB에 기록되어 이후 모든 클라이언트 작업에 반영된다.
