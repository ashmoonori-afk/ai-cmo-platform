...
- 11개 specialist role과 55개 playbook으로 실행합니다.
...
| 감사 + 로드맵 (현 상태의 냉정한 평가) | `docs/system/pipeline-audit-and-roadmap-2026-06-25.md` |
...
uv run pytest -q        # 91 tests, 동작 확인
```
...
| SEO 감사 루브릭 + 깨진 참조 | `seo-audit.md` | 가중치 35/35/30; **GEO 참조 경로 불일치**(`Sample Client A-GEO-실행가이드.md` ↔ 실제 `references/sample-client-a-geo-guide.md`) → 경로 수정·가중치 조정 |
...
| 깨진 참조 | `performance-report.md`, `ga-audit.md` | 존재하지 않는 `ga-optimizer/CLAUDE.md` 참조 → 추가하거나 원칙을 inline |
