...
| Merge to main | `main` includes embedded skills and prior `origin/main` history without force push. | pending |
...
| ESM-002 | medium | Branch topology | `origin/main` and `master` had unrelated histories. | Merge with unrelated histories while preserving `origin/main` content. | `git log --graph --all` and final push to `main`. | pending |
