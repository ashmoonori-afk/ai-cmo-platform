...
함께 제공해야 실행하며, URL·확인일·지원되는 개인정보 패턴을 마스킹한 본문의 SHA-256을
`aicmo.source-manifest.v1` 파일에 기록한다. 합성 검사에서 인식되는 고객 이름·전화·이메일이
SQLite 기록, 모델 요청, 중간 파일과 결과 파일에 남지 않음을 확인했다. 법적 익명화나
자유 서술의 모든 개인정보 제거를 보장하지 않는다. 공개 매장 전화는 별도 입력으로
...
| 원문+URL+`YYYY-MM-DD` 확인일 | KST 운영일 검증·지원 패턴 마스킹 후 실행, `source-manifest.json` 생성 |
