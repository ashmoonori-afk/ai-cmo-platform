...
1. outputs/{client}/ 하위 모든 폴더를 Glob으로 스캔 (최근 7일/30일)
   파일 날짜 판별 순서:
   - (1) 파일명의 YYYYMMDD_ 접두사를 파싱한다
   - (2) 접두사가 없거나 파싱 불가능한 경우, 파일 시스템의 수정일을 사용한다
   - (3) 날짜를 판별할 수 없는 파일은 [날짜 미상] 태그와 함께 리포트에 포함하되, 핵심 발견 도출에서는 제외한다
...
**기간**: {시작일} ~ {종료일}
**생성일**: {YYYY-MM-DD}
**스캔 결과**: {N}개 파일 발견 / {M}개 기간 내 / {K}개 날짜 미상
