...
- `clients/{client}/config.md` — 브랜드별 데이터 기준 (날짜 포맷, 통화, 지역)
...
| 행 번호 | 컬럼 | 변환 유형 | 원본값 | 변환값 |
|--------|-----|---------|------|------|
| {row} | {col} | {type} | {before} | {after} |

> 전체 변환 로그: `outputs/{client}/operations/data-cleanup-log-{YYYYMMDD}.csv`
...
```
outputs/{client}/operations/data-cleanup-{YYYYMMDD}.md      (리포트)
outputs/{client}/operations/data-cleanup-log-{YYYYMMDD}.csv (변환 로그)
{OUTPUT_FILE}                                               (정리된 데이터)
```
