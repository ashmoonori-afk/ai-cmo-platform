...
- `RAW_NOTES`는 실명, 전화번호, 이메일, 주민/사업자번호, 계정 ID, 쿠키, 토큰, API 키, 인증 헤더를 제거하거나 역할/이니셜로 마스킹한 사본만 사용한다.
- 회의록은 전체 원문이나 민감 발언을 보존하지 않고 안건별 요약, 결정 사항, 액션 아이템만 남긴다.
- 참석자 표에는 필요한 최소 식별자만 남기고, 외부 공유 가능성이 있으면 역할명 또는 이니셜을 사용한다.
...
- 수치/날짜/이름 등 구체적 정보는 필요한 최소 수준으로 보존
