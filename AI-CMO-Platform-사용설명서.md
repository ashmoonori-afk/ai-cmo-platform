...
> 11개 AI 에이전트 + 33개 플레이북으로 마케팅팀을 대체하는 노코드 AI 마케팅 플랫폼
...
- **자동 품질 검증**: reviewer 에이전트가 모든 산출물을 검수
- **지식 축적**: 사용할수록 클라이언트를 더 잘 이해하게 됨
...
모든 산출물은 `outputs/{클라이언트명}/{모듈}/` 폴더에 자동 저장됩니다.
...
| **Designer** | 디자이너 | 랜딩페이지/상세페이지 디자인 디렉션, 디자인 감사, 전환 최적화 | sonnet |
| **Repurposer** | 콘텐츠 변환기 | 하나의 콘텐츠를 LinkedIn, Twitter, 뉴스레터, 카드뉴스 4개 포맷으로 변환 | haiku |
| **SEO Specialist** | SEO 전문가 | 키워드 리서치, SEO/GEO 감사, 콘텐츠 최적화 제안 | sonnet |
...
| **Reporter** | 리포터 | 주간/월간 리포트 생성 + knowledge-base 업데이트 관리 | sonnet |
...
| 29 | **Weekly Report** | "주간 리포트 만들어줘" | reporter | 모듈별 활동 요약 + 핵심 발견 + 다음 주 추천 액션 + KB 업데이트 |
...
"새 클라이언트 온보딩: {회사명}, 웹사이트: {URL}"
```
→ 웹 검색으로 정보 수집 → 4개 파일 자동 생성 → 초기 분석 포함
...
├── insights.md          ← 리서치 인사이트 자동 누적
├── winning-copy.md      ← 성과 좋은 카피 자동 저장
...
# 금요일
"주간 리포트 만들어줘"      → 이번 주 성과 정리 + KB 업데이트
```
...
# 미팅 후
"미팅 정리해줘 — {미팅 메모 붙여넣기}"    → CRM 요약 + 팔로업 이메일
```
...
│   ├── strategist.md            ← 전략가
│   ├── copywriter.md            ← 카피라이터
│   ├── designer.md              ← 디자이너 (랜딩/상세페이지, 디자인 감사)
...
│   └── knowledge-update.md      ← KB 업데이트 규칙
│
...
### Q: 에이전트가 틀린 내용을 생성하면?
→ reviewer 에이전트가 자동 검증합니다. FAIL 판정 시 최대 2회 재실행. 그래도 안 되면 사용자에게 보고합니다.

### Q: 비공개 데이터(매출, GA)는 어떻게?
→ CSV/Excel 파일을 프로젝트 폴더에 넣고 "이 데이터 분석해줘" 라고 하면 data-analyst가 처리합니다.
...
### Q: 비용은 얼마나 드나요?
→ Claude Code 구독료만. 에이전트 실행 시 Claude API 토큰을 사용합니다. 병렬 실행과 haiku 모델 활용으로 비용 최적화되어 있습니다.
...
> **AI CMO Platform v1.0** — 11 Agents, 7 Modules, 33 Playbooks, 48 Workflows
