...
> 11개 AI 에이전트 + 67개 플레이북으로 마케팅 반복 업무를 표준화하는 노코드 AI 마케팅 운영체계
...
3. [기본 사용법](#3-기본-사용법)
4. [에이전트 구성 (11개)](#4-에이전트-구성-11개)
5. [SOP/플레이북 목록](#5-sop플레이북-목록)
...
- **노코드**: 코딩 없이 자연어로만 조작
- **11개 전문 에이전트**: 각 영역의 전문가 역할 수행
- **67개 플레이북(SOP)**: 검증된 마케팅 프레임워크 내장 (코어 42 + 체인 5 + 역할 SOP 10 + 로컬 실행 7 + 유료 광고 3)
- **런치팩 체인**: 상담 한 번 → 전략·채널믹스·후킹문안·브랜드킷·홈페이지 5종을 순서대로 자동 산출
- **로컬 실행 SOP**: 네이버 플레이스·카톡채널·당근·배달앱·오프라인 — 붙여넣기만 하면 되는 "대행 수준" 실행 팩 (2026-07 시장 리서치 근거)
...
| **Designer** | 디자이너 | 랜딩페이지/상세페이지 디자인, 브랜드 비주얼 방향, 디자인 감사 | sonnet |
...
> 아래는 자주 쓰는 핵심 플레이북입니다. 전체 67개(디자인 5, 체인 5, 역할 SOP 10, 로컬 실행 7, 유료 광고 3 포함)는 `playbooks/` 폴더에서 확인할 수 있습니다.

### 모듈 00: Chains (복합 체인) — 5개

| # | 플레이북 | 명령어 예시 | 에이전트 조합 | 산출물 |
|---|---------|-----------|-------------|--------|
| C1 | **Launch Pack (런치팩)** | "사업 시작 마케팅 전부 해줘", "런치팩" | 상담 온보딩 → researcher + competitor → strategist → copywriter → designer → reviewer → reporter | 5대 산출물: ①마케팅 전략(90일) ②채널믹스 ③후킹문안 리스트 ④브랜드 에셋 킷 ⑤홈페이지 카피+HTML — 상담 한 번에서 순서대로 |
| C2 | **Full Strategy Chain** | "전체 마케팅 전략" | researcher + competitor → strategist → reviewer | GTM → 포지셔닝 → 채널 → 퍼널 종합 전략 보고서 |
| C3 | **Content Full Cycle** | "블로그 쓰고 퍼뜨려줘" | seo-specialist → copywriter → repurposer → reviewer | 블로그 + 4개 채널 변환본 |
| C4 | **Sales Prep Bundle** | "영업 준비 풀세트" | researcher → sales-writer → reviewer | 기업 조사 + 미팅 브리프 + 콜드메일 시퀀스 |
| C5 | **AI CMO Operating System** | (시스템 내부 파이프라인) | 전 에이전트 | 인테이크 → 명확화 → 실행 → 검증 → 리포트 표준 흐름 |
...
### 모듈 09: Local (로컬 실행 — "대행 수준" 실행 팩) — 7개

| # | 플레이북 | 명령어 예시 | 에이전트 조합 | 산출물 |
|---|---------|-----------|-------------|--------|
| L1 | **네이버 플레이스** | "플레이스 등록해줘" | seo-specialist → copywriter → reviewer | 설명문 300자 + 대표키워드 5 + 소식 4주분 + 리뷰 답글 10종 + 월간 리포트 틀 |
| L2 | **카카오톡 채널** | "단골 관리 하고 싶어" | copywriter → reviewer | 개설 시트 + 무상캐시 30만 원 신청 + 메시지 캘린더 3개월 + 친구모으기 키트 |
| L3 | **당근 비즈프로필** | "당근에 가게 올려줘" | copywriter → reviewer | 프로필 시트 + 소식 4주분 + 쿠폰 2종 + 동네 광고 세팅안 |
| L4 | **배달앱 최적화** | "배민 입점 도와줘" | copywriter → reviewer | 서류 체크 + 가게소개·메뉴 문안 전량 + 적법 리뷰 이벤트 + 답글 10종 |
| L5 | **SNS 최소 루틴** | "SNS 시작하고 싶어" | strategist → copywriter → reviewer | 채널 진단 1장 + 주간 루틴 + 소재 12개 + 15초 스크립트 |
| L6 | **오프라인 인쇄물** | "전단지 만들어줘" | copywriter → designer → reviewer | 합법성 체크(신고·과태료) + 전단/현수막 카피 + 발주 사양서 |
| L7 | **지원사업+사기방어** | "정부 지원 뭐 있어?", "광고 전화 받았는데" | researcher → reporter | 신청 체크리스트 + 대행 사기 감별표 + 정상 시세표 |

### 모듈 10: Ads (유료 광고 — 실전 운영) — 2개

| # | 플레이북 | 명령어 예시 | 에이전트 조합 | 산출물 |
|---|---------|-----------|-------------|--------|
| A1 | **네이버 검색광고** | "파워링크 해줘", "네이버 광고" | seo-specialist → copywriter → reviewer | 캠페인 구조도 + 키워드 시트 30-50개(3분류·입찰가) + T&D 5세트 + 전환추적 체크 + 주간 10분 루틴 + 사기 방어 1장 |
| A2 | **메타 광고** | "인스타 광고 돌려줘", "페이스북 광고" | strategist → copywriter → reviewer | 계정 세팅 체크(픽셀·CAPI) + 예산 구간별 구조안 + 소재 6컨셉 + 끄기·증액 룰 + 정책 사전 심사 |
| A3 | **광고 전략 라이브러리** | "광고 실험 어떻게 해?", "광고 지표 해석" | (참조 사전) | 검증 전략 20종 — 실험 설계 5(킬 룰·2캠페인·80/20·동일비중·On/Off 증분) + 지표 해석 5(MER·판단 유예·진단 트리·오독 6종·윈도 고정) + 컨텐츠 5(앵글 맵·다양화·UGC·125자·리프레시) + 후킹 5(3초 룰·결과 먼저·텍스트 공식·모듈식·한국형) |
...
### 예시 0: 런치팩 — 상담 한 번으로 5종 산출물 (권장 기본 경로)

사업을 처음 시작하는 분(특히 마케팅 용어가 낯선 50-60대 사장님)은 이 한 줄이면 됩니다:

```
"런치팩 해줘 — 새 클라이언트: {회사명}"
```

진행 순서 (자동): 쉬운 질문 10개 상담 → 시장 확인 → ①마케팅 전략(90일)
→ ②채널믹스 → ③후킹문안 리스트 → ④브랜드 에셋 킷 → ⑤홈페이지 시안
→ 검수 후 "다음 2주 할 일 3개"가 담긴 리포트로 마무리.

중간에 확인할 것은 단 두 번: 상담 내용 요약 확인, 그리고 최종 승인.

### 예시 1: 신규 사업 런칭 (하루 작업, 단계별 수동 실행)
...
│
├── playbooks/                   ← 실행 가능한 SOP (67개)
│   ├── 00-chains/               (5) 복합 체인 (launch-pack 포함)
│   ├── 01-strategy/             (7) 전략 기획
│   ├── 02-intelligence/         (5) 리서치/분석
│   ├── 03-content/              (10) 콘텐츠 제작
│   ├── 04-sales/                (5) 세일즈 지원
│   ├── 05-seo/                  (4) SEO
│   ├── 06-analytics/            (3) 성과 분석
│   ├── 07-operations/           (3) 운영
│   ├── 08-design/               (5) 디자인
│   ├── 08-role-sops/            (10) 역할별 SOP
│   ├── 09-local/                (7) 로컬 실행 (플레이스/카톡/당근/배달앱/SNS/오프라인/지원)
│   └── 10-ads/                  (3) 유료 광고 (네이버 검색광고/메타광고/전략 라이브러리)
│
